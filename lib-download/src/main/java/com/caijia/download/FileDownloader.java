    ...
    private void realDownload(FileResponse response) {
        ...
        while (true) {
            if (a != 1) {
                ...
                if (split[0].contains("(")) {
                    fileName = split[0].substring(0, split[0].indexOf("(")) + "(" + a + ")." + split[1];
                } else {
                    fileName = split[0] + "(" + a + ")." + split[1];
                }
            }
            if (!hashSet.contains(fileName)) {
                fileReleaseName = fileName;
                break;
            } else {
                a++;
            }
        ...
        for (int i = 0; i < threadCount; i++) {
            DownloadCallable callable = new DownloadCallable(
                    saveFileDir, fileReleaseName, fileSize, connection, fileRequest, i,
