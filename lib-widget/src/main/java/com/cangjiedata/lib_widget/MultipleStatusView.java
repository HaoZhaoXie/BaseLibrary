...
public class MultipleStatusView extends RelativeLayout {
    ...
    public static final int STATUS_NO_FRIEND = 0x05;
    ...
    private int mNoFriendViewResId;
    ...
    public MultipleStatusView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mEmptyViewResId = a.getResourceId(R.styleable.MultipleStatusView_emptyView, R.layout.layout_widget_empty_view);
        mErrorViewResId = a.getResourceId(R.styleable.MultipleStatusView_errorView, R.layout.layout_widget_error_view);
        mLoadingViewResId = a.getResourceId(R.styleable.MultipleStatusView_loadingView, R.layout.layout_widget_loading_view);
        mNoNetworkViewResId = a.getResourceId(R.styleable.MultipleStatusView_noNetworkView, R.layout.layout_widget_no_network_view);
        mNoFriendViewResId = a.getResourceId(R.styleable.MultipleStatusView_noFriendView, R.layout.layout_widget_no_friend_view);
     ...
     * 自定义 显示空视图
     */
    public final void showEmptyCustom(int mEmptyViewResId) {
    ...
    public final void showNoFriendView() {
        showNoFriendView(mNoFriendViewResId, DEFAULT_LAYOUT_PARAMS);
    }
     ...
     * @param formatArgs 占位符参数
     */
    public final void showNoFriendView(int hintResId, Object... formatArgs) {
        showNoFriendView();
        setStatusHintContent(mNoFriendView, hintResId, formatArgs);
    }
     ...
     * @param hint 自定义提示文本内容
     */
    public final void showNoFriendView(String hint) {
        showNoFriendView();
        setStatusHintContent(mNoFriendView, hint);
    }


    public final void showNoFriendView(int layoutId, ViewGroup.LayoutParams layoutParams) {
        showNoFriendView(null == mNoFriendView ? inflateView(layoutId) : mNoFriendView, layoutParams);
    }
     ...
     * @param layoutParams 布局参数
     */
    public final void showNoFriendView(View view, ViewGroup.LayoutParams layoutParams) {
        checkNull(view, "Friend view is null!");
        checkNull(layoutParams, "Layout params is null.");
        changeViewStatus(STATUS_NO_FRIEND);
        if (null == mNoFriendView) {
            mNoFriendView = view;
            View noFriendView = mNoFriendView.findViewById(R.id.no_friend_view);
            if (null != mOnRetryClickListener && null == noFriendView) {
                noFriendView.setOnClickListener(mOnRetryClickListener);
            }
            mOtherIds.add(mNoFriendView.getId());
            addView(mNoFriendView, 0, layoutParams);
        }
        showViewById(mNoFriendView.getId());
    }
