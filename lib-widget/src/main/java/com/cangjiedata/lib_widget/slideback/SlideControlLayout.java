...
import com.cangjiedata.lib_widget.WidgetUtils;
...
public class SlideControlLayout extends FrameLayout {
    ...
    private void onBack() {
        if (onSlide == null) {
            WidgetUtils.getActivityContext(getContext()).onBackPressed();
        } else {
            onSlide.onSlideBack();
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent motionEvent) {
        ...
        switch (motionEvent.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (currentY > WidgetUtils.dip2px(getContext(), 100) && currentX <= canSlideWidth) {
                    downX = currentX;
                    startDrag = true;
