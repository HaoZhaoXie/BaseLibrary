    ...
    public CharSequence filter(CharSequence source, int start, int end, Spanned dest, int dstart,
        ...
        if (destText.contains(POINTER)) {
            ...
            if (!matcher.matches()) {
                ...
                if ((POINTER.equals(sourceText)) && TextUtils.isEmpty(destText)) {  //首位不能输入小数点
                    return "";
                } else if (!POINTER.equals(sourceText) && ZERO.equals(destText)) { //如果首位输入0，接下来只能输入小数点
                    return "";
                }
            }
        }
