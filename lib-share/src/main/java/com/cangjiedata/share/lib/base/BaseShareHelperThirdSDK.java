package com.cangjiedata.share.lib.base;
...
import com.cangjiedata.share.lib.BuildConfig;
import com.cangjiedata.share.lib.ShareConstant;
import com.cangjiedata.share.lib.ShareUtil;
import com.cangjiedata.share.lib.bean.BaseShareContent;
import com.cangjiedata.share.lib.listener.OnShareHelperListener;
import com.cangjiedata.share.lib.listener.SimpleCallBack;
...
public class BaseShareHelperThirdSDK extends BaseShareHelperImageGetter {
    ...
    protected String miniProgramId = "gh_995ee341f272";//小程序ID
    ...
    public void shareByMiniProgram(final Activity activity, final BaseShareContent shareContent, final int shareType, OnShareHelperListener onShareHelperListener) {
        ...
        Glide.with(activity)
                .asFile()
                .load(shareContent.getPictureResource())
                .listener(new RequestListener<File>() {
                    ...
                    public boolean onResourceReady(File resource, Object model, Target<File> target, DataSource dataSource, boolean isFirstResource) {
                        luBan(activity, shareContent, shareType, resource);
                        return false;
                    }
                })
                .preload(ShareConstant.THUMB_WIDTH_SIZE, ShareConstant.THUMB_HEIGHT_SIZE);
    }

    private void luBan(Activity activity, BaseShareContent shareContent, int shareType, File file) {
    ...
    private void shareTextQQ(Activity activity, BaseShareContent shareContent, int shareType) {
        ...
        if (super.onShareHelperListener != null) {
            super.onShareHelperListener.onComplete("qq");
        }
    }
    ...
    private void shareTextWX(BaseShareContent shareContent, int shareType) {
        ...
        wxApi.sendReq(req);
        if (super.onShareHelperListener != null) {
            super.onShareHelperListener.onComplete(shareType == 0 ? "wechat" : "moments");
        }
    }
    ...
    private void sharePictureWX(Activity activity, BaseShareContent shareContent, int shareType, File file) {
        Bitmap bitmap = file == null ? ShareUtil.getThumbnail(null, activity, shareContent.getDefaultPictureResource()) : ShareUtil.decodeSampledBitmapFromFile(file.getPath(), ShareConstant.THUMB_SIZE, ShareConstant.THUMB_SIZE);
    ...
    private void shareToMiniProgram(Activity activity, BaseShareContent shareContent, int shareType, File file) {
        Bitmap bitmap = file == null ? ShareUtil.getThumbnail(null, activity, shareContent.getDefaultPictureResource()) : ShareUtil.decodeSampledBitmapFromFile(file.getAbsolutePath(), ShareConstant.THUMB_WIDTH_SIZE, ShareConstant.THUMB_HEIGHT_SIZE);
        WXMiniProgramObject miniProgramObj = new WXMiniProgramObject();
        miniProgramObj.webpageUrl = shareContent.getURL();                             // 兼容低版本的网页链接
        miniProgramObj.miniprogramType =
                BuildConfig.IS_TEST ? WXMiniProgramObject.MINIPROGRAM_TYPE_PREVIEW
        ...
        if (super.onShareHelperListener != null) {
            super.onShareHelperListener.onComplete("wechatMiniProgram");
        }
    }
