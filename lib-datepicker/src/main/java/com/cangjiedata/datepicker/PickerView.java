...
public class PickerView extends View {
    ...
    private void drawText(Canvas canvas, int textColor, float offsetY, String text) {
        ...
        double contentWith = (getResources().getDisplayMetrics().widthPixels - getResources().getDisplayMetrics().density * 32 - 0.5);
        if (rect.width() > contentWith) {
            StringBuilder stringBuilder = new StringBuilder();
            for(int i = 0; i < text.length(); i++){
                stringBuilder.append(text.charAt(i));
                Rect sbR = new Rect();
                mPaint.getTextBounds(stringBuilder.toString(), 0, stringBuilder.length(), sbR);
                if (sbR.width() > contentWith) {
                    if(stringBuilder.length() > 4){
                        text = stringBuilder.substring(0, stringBuilder.length() - 4) + "...";
                    }else {
